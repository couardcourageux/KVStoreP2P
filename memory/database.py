...
class MockDatabase:
    ...
    def popEntry(self, key:str, user_id:str="12") -> Response:
        entry = self.__data.get(key, None)
        if entry is None:
            return Response(status=1, data=f"{key} not found", errors=[KeyNotFound(key)])
            ...
            if entry.public_write == 0:
                ...
                if entry.author_id == user_id:
                    ...
                    return Response(status=1, data=f"{key} failed to be deleted", errors=[UnauthorizedWriting(key, user_id)])
        return Response(status=0, data=f"{key} successfully deleted")
